      ...
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: { md: 'center' }, justifyContent: 'space-between', gap: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Box>
            ...
            <Typography style={{ fontWeight: 600, color: '#111827' }}>Auto-imported receipts from Gmail</Typography>
          </Box>
        </Box>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
          ...
          <Box component="button" onClick={() => router.push('/integrations/gmail')} sx={{ display: 'inline-flex', alignItems: 'center', gap: 1, border: '1px solid #e5e7eb', px: 2, py: 1, fontSize: 14, fontWeight: 600, color: '#374151', bgcolor: 'transparent', cursor: 'pointer', '&:hover': { bgcolor: '#f9fafb' } }}>
...
export function FolderContextMenu({ folderContextMenu, setFolderContextMenu, setFolderTagPickerId, handleStartEditFolder, confirmDeleteFolder, tagsTitle, renameTooltip, deleteTooltip }: FolderContextMenuProps): React.JSX.Element | null {
  ...
  return (
    <Box sx={{ position: 'fixed', zIndex: 100, minWidth: 200, overflow: 'hidden', border: '1px solid #e5e7eb', bgcolor: 'background.paper', boxShadow: 24 }} style={{ top: topOffset, left: leftOffset }} onClick={e => e.stopPropagation()} onKeyDown={e => e.stopPropagation()} role="presentation">
      <Box sx={{ p: 0.75, display: 'flex', flexDirection: 'column' }}>
        <Box component="button" type="button" onClick={() => { setFolderTagPickerId(folderContextMenu.folder.id); setFolderContextMenu(null); }} sx={{ display: 'flex', width: '100%', alignItems: 'center', gap: 1, px: 1.5, py: 1, fontSize: 14, color: '#374151', bgcolor: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', '&:hover': { bgcolor: '#f9fafb' } }}>
          <span>{tagsTitle}</span>
        </Box>
        <Box component="button" type="button" onClick={() => { handleStartEditFolder(folderContextMenu.folder); setFolderContextMenu(null); }} sx={{ display: 'flex', width: '100%', alignItems: 'center', gap: 1, px: 1.5, py: 1, fontSize: 14, color: '#374151', bgcolor: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', '&:hover': { bgcolor: '#f9fafb' } }}>
          <span>{renameTooltip}</span>
        </Box>
        <Box component="button" type="button" onClick={() => { confirmDeleteFolder(folderContextMenu.folder); setFolderContextMenu(null); }} sx={{ display: 'flex', width: '100%', alignItems: 'center', gap: 1, px: 1.5, py: 1, fontSize: 14, color: '#dc2626', bgcolor: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', '&:hover': { bgcolor: '#fef2f2' } }}>
