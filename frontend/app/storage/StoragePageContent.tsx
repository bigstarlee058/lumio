  ...
  return (
    <DndContext sensors={dndHook.sensors} collisionDetection={pointerWithin} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <Box className="container-shared" sx={{ px: { xs: 2, sm: 3, lg: 4 }, py: 4 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 3, py: 2, bgcolor: 'var(--muted)', border: '1px solid var(--border-color)', mb: 0 }}>
          <Typography style={{ fontSize: 18, fontWeight: 600, color: 'var(--foreground)' }}>{isTrashView ? t.trash.title : t.subtitle}</Typography>
        ...
        <DragOverlay style={{ pointerEvents: 'none' }}>
          {dndHook.draggingFile ? (
            <Box sx={{ bgcolor: 'background.paper', p: 2, border: '1px solid rgba(22,129,24,0.5)', opacity: 0.9, width: 300 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                ...
                <Box sx={{ minWidth: 0, flex: 1 }}>
                  ...
                  <Box style={{ fontSize: 12, color: 'var(--muted-foreground)' }}>{formatFileSize(dndHook.draggingFile.fileSize)}</Box>
                </Box>
              </Box>
            </Box>
          ) : null}
