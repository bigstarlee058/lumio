...
export default function Navigation() {
  ...
  const { nav, userMenu, languageModal, languages: languageNames, tour } = useIntlayer('navigation');
  ...
  const { mobileMenuOpen, setMobileMenuOpen, mobileMenuMounted, mobileMenuVisible } = useMobileMenu(pathname);
  const { selectedTheme, handleThemePreferenceChange } = useThemePreference({ userThemePreference: user?.themePreference, user, setUser, setTheme, resolveLabel, navTheme: getRecord(getRecord(nav)?.theme)?.description });
  const langProps = useLanguageSelection({ locale, availableLocales, setLocale, languageNames, languageModal, setMobileMenuOpen });
  ...
  const navigateFromUserMenu = useCallback((path: string): void => {
    setMobileMenuOpen(false); router.push(path);
  ...
  const handleAction = useCallback((key: string): void => {
    const MAP: Record<string, () => void> = {
      ...
      knowledgeBase: () => { setMobileMenuOpen(false); window.open('https://symonbaikov.github.io/lumio/', '_blank', 'noopener,noreferrer'); },
      logout: () => { setMobileMenuOpen(false); void logout(); toast.success(userMenu.logoutSuccess.value); },
    };
    MAP[key]?.();
        ...
        <div className="lumio-navigation__bar">
          <div className="lumio-navigation__brand">
            ...
            <nav className="lumio-navigation__primary-nav" data-tour-id="primary-nav">
              {visibleNavItems.map(item => {
                ...
                return (
                  <Link key={item.path} href={item.path} className={`lumio-navigation__nav-item${active ? ' lumio-navigation__nav-item--active' : ''}`}>
          ...
          <div className="lumio-navigation__actions">
            <div className="lumio-navigation__desktop-actions">
              ...
              <TourMenu trigger={<button type="button" className="lumio-navigation__icon-btn" title="Help"><HelpCircle size={20} /></button>} />
              <div className="lumio-navigation__user-menu-area"><UserMenuTriggerAndDropdown {...userMenuProps} /></div>
            </div>
            <div className="lumio-navigation__mobile-toggle">
              <button onClick={() => { setMobileMenuOpen(prev => !prev); }} className="lumio-navigation__mobile-btn" data-tour-id="mobile-menu-toggle" aria-label="Open menu" aria-expanded={mobileMenuOpen}>
                {mobileMenuOpen ? <X size={24} /> : <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="3" y1="12" x2="21" y2="12" /><line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="18" x2="21" y2="18" /></svg>}
              </button>
            </div>
          </div>
        </div>
      </div>
      {mobileMenuMounted && <MobileDrawer mobileMenuVisible={mobileMenuVisible} visibleNavItems={visibleNavItems} pathname={pathname} setMobileMenuOpen={setMobileMenuOpen} selectedTheme={selectedTheme} handleThemePreferenceChange={handleThemePreferenceChange} userMenuProps={userMenuProps} nav={nav} />}
      {portalReady && <LanguageDrawer isOpen={langProps.languageModalOpen} onClose={langProps.closeLanguageMenu} languageModal={languageModal} languageSearch={langProps.languageSearch} setLanguageSearch={langProps.setLanguageSearch} filteredLanguages={langProps.filteredLanguages} normalizedLocale={langProps.normalizedLocale} handleLanguageSelect={langProps.handleLanguageSelect} />}
    </header>
  );
}
