...
function ExportMenu({ labels, onDownloadExport }: ExportMenuProps): React.ReactElement {
  return (
    <Box sx={{ position: 'absolute', right: 0, zIndex: 10, mt: 0.5, width: 144, border: '1px solid var(--border)', bgcolor: 'var(--card)', p: 0.5, boxShadow: 1 }}>
      <button type="button" style={menuBtnStyle} onClick={() => onDownloadExport('excel')}>{labels.exportExcel}</button>
      <button type="button" style={menuBtnStyle} onClick={() => onDownloadExport('pdf')}>{labels.exportPdf}</button>
    </Box>
  );
}

type FilterControlsProps = { filterMode: 'now' | 'date'; selectedDate: string; exportingFormat: BalanceExportFormat | null; labels: BalanceFilterLabels; onFilterModeChange: (mode: 'now' | 'date') => void; onDateChange: (date: string) => void; onRefresh: () => void };
function FilterControls({ filterMode, selectedDate, exportingFormat, labels, onFilterModeChange, onDateChange, onRefresh }: FilterControlsProps): React.ReactElement {
  ...
  const handleDatePickerChange = (d: Date | null): void => onDateChange(d && isValid(d) ? format(d, 'yyyy-MM-dd') : '');
  return (
    <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 1 }}>
      <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 1, border: '1px solid var(--border)', bgcolor: 'var(--card)', px: 1.5, py: 1, fontSize: 14, color: 'var(--foreground)' }}>
      ...
      {filterMode === 'date' && <DatePicker value={dateValue} onChange={handleDatePickerChange} slotProps={{ textField: { size: 'small' } as never }} />}
      <button type="button" onClick={onRefresh} disabled={!!exportingFormat} style={{ ...buttonBase, color: 'var(--muted-foreground)' }}>
        <RefreshCcw size={16} />{labels.refresh}
      </button>
    </Box>
  );
}

export function BalanceFilterBar({ filterMode, selectedDate, exportingFormat, exportMenuOpen, saveHint, labels, onFilterModeChange, onDateChange, onRefresh, onExportMenuToggle, onDownloadExport }: BalanceFilterBarProps): React.ReactElement {
  return (
    <Box sx={{ border: '1px solid var(--border)', bgcolor: 'var(--card)', p: 2 }}>
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: { md: 'center' }, justifyContent: { md: 'space-between' }, gap: 1.5 }}>
        <FilterControls filterMode={filterMode} selectedDate={selectedDate} exportingFormat={exportingFormat} labels={labels} onFilterModeChange={onFilterModeChange} onDateChange={onDateChange} onRefresh={onRefresh} />
        <Box sx={{ position: 'relative' }}>
          <button type="button" onClick={onExportMenuToggle} disabled={!!exportingFormat} style={{ ...buttonBase, color: 'var(--foreground)' }}>
      ...
      {saveHint && (
        <Typography variant="caption" sx={{ mt: 1, display: 'block', color: 'var(--muted-foreground)' }}>{saveHint}</Typography>
      )}
    </Box>
  );
}
