...
type EmptyStateProps = { createLabel: string; noWorkspacesLabel: string; onCreateClick: () => void };
function EmptyWorkspacesState({ createLabel, noWorkspacesLabel, onCreateClick }: EmptyStateProps): React.JSX.Element {
  return (
    <Box sx={{ textAlign: 'center', py: 6 }}>
      <Box sx={{ fontSize: 48, mb: 2 }}><Building2 size={48} /></Box>
      <Typography variant="h6" fontWeight={600} sx={{ mb: 1, color: 'var(--foreground)' }}>{noWorkspacesLabel}</Typography>
      <Typography variant="body2" sx={{ mb: 3, color: 'var(--text-secondary)' }}>Create your first workspace to get started</Typography>
...
function NoResultsState(): React.JSX.Element {
  return (
    <Box sx={{ textAlign: 'center', py: 6 }}>
      <Box sx={{ mb: 2 }}><Search size={48} /></Box>
      <Typography variant="h6" fontWeight={600} sx={{ mb: 1, color: 'var(--foreground)' }}>No workspaces found</Typography>
      <Typography variant="body2" sx={{ mb: 3, color: 'var(--text-secondary)' }}>Try adjusting your search query</Typography>
    </Box>
  );
}
...
export function WorkspaceGridView({ workspaces, allWorkspacesEmpty, filteredEmpty, createLabel, noWorkspacesLabel, onWorkspaceClick, onCreateClick, onFavoriteToggle }: WorkspaceGridViewProps): React.JSX.Element {
  if (allWorkspacesEmpty) return <EmptyWorkspacesState createLabel={createLabel} noWorkspacesLabel={noWorkspacesLabel} onCreateClick={onCreateClick} />;
  if (filteredEmpty) return <NoResultsState />;
  return (
