  ...
  return (
    <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 1.5, border: '1px dashed var(--border)', borderRadius: tokens.radius.md, px: 2, py: 1.5 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        <Button size="small" variant="outlined" disabled={!isOwnerOrAdmin || isActionBusy} onClick={() => void onResend(invite)}>
          {isResending ? 'Sending...' : 'Resend'}
        </Button>
        <Button size="small" variant="outlined" color="error" disabled={!isOwnerOrAdmin || isActionBusy} onClick={() => void onRevoke(invite.id)}>
          {isRevoking ? 'Revoking...' : 'Revoke'}
        </Button>
        <Button size="small" variant="text" disabled={isActionBusy} onClick={() => void onCopyLink({ token: invite.token, link: invite.link })}>
          Copy link
        </Button>
      </Box>
    </Box>
