...
export default function TopMerchantsView() {
  ...
  const labels = {
    ...
    subtitle: resolveLabel(
      (t as any)?.topMerchants?.subtitle,
      'Spending analytics by merchants and counterparties with Submit filters.',
    ),
    searchPlaceholder: resolveLabel(
      (t as any)?.topMerchants?.searchPlaceholder,
      'Search by merchant, sender or subject',
    ),
    totalSpend: resolveLabel((t as any)?.topMerchants?.totalSpend, 'Total spend'),
    statementsSpend: resolveLabel((t as any)?.topMerchants?.statementsSpend, 'Statements'),
    ...
    amount: resolveLabel((t as any)?.topMerchants?.amount, 'Amount'),
    operations: resolveLabel((t as any)?.topMerchants?.operations, 'Operations'),
    average: resolveLabel((t as any)?.topMerchants?.average, 'Average'),
  ...
  useEffect(() => {
    const storedFilters = loadTopMerchantsFilters();
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        console.error('Failed to load top merchants data', error);
        if (isMounted) {
  ...
  const applyFilterChanges = () => {
    ...
    saveTopMerchantsFilters(draftFilters);
  };
  ...
  const resetAllFilters = () => {
    ...
    saveTopMerchantsFilters(DEFAULT_STATEMENT_FILTERS);
  };
