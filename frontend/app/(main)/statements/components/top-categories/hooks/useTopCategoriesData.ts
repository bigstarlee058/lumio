import { useTopCategoriesAggregation, type TopCategoriesAggregationReturn } from '@/app/(main)/statements/components/top-categories/hooks/useTopCategoriesAggregation';
import { useTopCategoriesRecords, type CategoryFromOption } from '@/app/(main)/statements/components/top-categories/hooks/useTopCategoriesRecords';
import type { useTopCategoriesState } from '@/app/(main)/statements/components/top-categories/hooks/useTopCategoriesState';
import type { TopCategoryAggregateRow, TopCategoryRecord } from '@/app/(main)/statements/components/top-categories.utils';
...
const filterDrillDownRecords = (selectedRow: TopCategoryAggregateRow, records: TopCategoryRecord[]): TopCategoryRecord[] => {
...
export const useTopCategoriesData = ({ user, currentWorkspace, workspaces, workspaceCurrency, resolvedTheme, labels, state }: Params): TopCategoriesDataReturn => {
  ...
  const { flowFilteredRecords, flowRecordsWithoutDateFilter, fromOptions, currencyOptions } = useTopCategoriesRecords({
  ...
  const { sortedAggregatedRows, totals, comparison, topCategoriesChart, sourceChart, trendChart } = useTopCategoriesAggregation({
    flowFilteredRecords,
  ...
  return { loading, flowFilteredRecords, fromOptions, currencyOptions, sortedAggregatedRows, totals, comparison, topCategoriesChart, sourceChart, trendChart, selectedRow, drillDownRecords };
};
