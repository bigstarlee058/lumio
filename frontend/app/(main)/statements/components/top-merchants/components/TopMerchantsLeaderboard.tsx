...
type ColumnLabels = { merchant: string; source: string; operations: string; average: string; amount: string; lastOperation: string };
...
type RowProps = { row: TopMerchantAggregateRow; sourceLabels: SourceLabels; onRowClick: (id: string) => void };
  ...
  return (
    <tr style={{ color: 'var(--foreground)', borderTop: '1px solid var(--muted)' }}>
      <td style={{ padding: '8px 16px 8px 0', fontWeight: 500, color: 'var(--foreground)' }}>
        <button type="button" style={{ textAlign: 'left', color: 'var(--primary)', background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontSize: 14 }} onClick={() => onRowClick(row.id)}>{row.merchant}</button>
      </td>
      <td style={{ padding: '8px 16px 8px 0' }}><AnalyticsSourceBadge sourceChannel={row.sourceChannel as TopMerchantSourceChannel} labels={sourceLabels} /></td>
...
export function TopMerchantsLeaderboard({ rows, sortKey, onSortChange, onRowClick, title, sourceLabels, sortLabels, columnLabels }: Props): React.JSX.Element {
        ...
        <table style={{ minWidth: '100%', fontSize: 14, borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ textAlign: 'left', fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--muted-foreground)' }}>
              <th style={{ padding: '8px 16px 8px 0' }}>{columnLabels.merchant}</th><th style={{ padding: '8px 16px 8px 0' }}>{columnLabels.source}</th>
