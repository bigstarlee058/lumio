...
type HeaderProps = { selectedRow: TopMerchantAggregateRow; sourceLabels: SourceLabels; labels: Pick<Props['labels'], 'drillDown' | 'close'>; onClose: () => void };
...
function DrillDownHeader({ selectedRow, sourceLabels, labels, onClose }: HeaderProps): React.JSX.Element {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--border-color)', padding: '12px 20px' }}>
      <div>
        <h4 style={{ fontSize: 14, fontWeight: 600, color: 'var(--foreground)' }}>{selectedRow.merchant} - {labels.drillDown}</h4>
        <p style={{ fontSize: 12, color: 'var(--muted-foreground)' }}>
          <AnalyticsSourceBadge sourceChannel={selectedRow.sourceChannel as TopMerchantSourceChannel} labels={sourceLabels} />
        </p>
      </div>
        ...
        {records.slice(0, 120).map(record => (
          <tr key={record.id} style={{ color: 'var(--foreground)', borderTop: '1px solid var(--muted)' }}>
            ...
            <td style={{ padding: '8px 16px 8px 0' }}>
              <AnalyticsSourceBadge sourceChannel={record.sourceChannel as TopMerchantSourceChannel} labels={sourceLabels} />
            </td>
...
export function TopMerchantsDrillDown({ selectedRow, drillDownRecords, onClose, currency, sourceLabels, labels }: Props): React.JSX.Element {
  return (
