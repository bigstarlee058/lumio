...
const extractMetaCurrency = (meta: Record<string, unknown> | null | undefined): string | undefined =>
  (meta?.currency as string | undefined) ?? (meta?.headerDisplay as Record<string, string> | undefined)?.currencyDisplay;

const extractParsedCurrency = (details: StatementFilterItem['parsingDetails']): string | undefined =>
...
type ParsedDataInput = { item: StatementFilterItem; company: string; sourceType: string; dateValue: string };
const buildStatementParsedData = ({ item, company, sourceType, dateValue }: ParsedDataInput): { vendor: string | null; date: string } => ({
...
const buildStatementRecord = ({ item, company, amount, currency, dateValue, sourceType, fileType, flow }: StatementRecordInput): TopSpenderRecord => ({
  id: item.id,
  source: sourceType,
  fileName: company,
...
export const mapStatementToRecord = (item: StatementWithWorkspace, workspaceCurrency: string): TopSpenderRecord => {
  const sourceType: 'statement' | 'gmail' = item.source === 'gmail' ? 'gmail' : 'statement';
  const company = sourceType === 'gmail' ? resolveGmailCompany(item) : getBankDisplayName(item.bankName);
  const dateValue = getStatementDate(item);
  const flow = resolveSpenderFlow({ sourceType, totalDebit: item.totalDebit, totalCredit: item.totalCredit });
  const currency = getStatementCurrency(item, workspaceCurrency);
  const fileType = (item.fileType ?? '').toLowerCase() || null;
  return buildStatementRecord({ item, company, amount: flow.amount, currency, dateValue, sourceType, fileType, flow });
};
...
const buildReceiptRecord = ({ receipt, company, currency, flow, receivedAt, parsedVendor, parsedDate, dateValue }: ReceiptRecordInput): TopSpenderRecord => ({
  id: receipt.id, source: 'gmail', fileName: company, subject: opt(receipt.subject), sender: opt(receipt.sender),
  status: opt(receipt.status), fileType: 'gmail', createdAt: receivedAt, statementDateFrom: parsedDate ?? receivedAt,
  statementDateTo: null, bankName: 'gmail', totalDebit: flow.amount, totalCredit: null, currency, exported: null, paid: null,
  parsingDetails: null, user: null, receivedAt, parsedData: { vendor: parsedVendor ?? company, date: parsedDate ?? receivedAt },
  company, amount: flow.amount, currencyValue: currency, dateValue, sourceType: 'gmail',
  sourceChannel: 'gmail' as TopSpenderSourceChannel, flowType: flow.flowType as TopSpenderFlowType,
  workspaceId: receipt.workspaceId, workspaceName: receipt.workspaceName,
});

export const mapReceiptToRecord = (receipt: GmailReceipt, workspaceCurrency: string): TopSpenderRecord => {
  const { vendor: parsedVendor, date: parsedDate, currency: parsedCurrency, amount: parsedAmount } = extractReceiptParsed(receipt);
  const company = resolveGmailMerchantLabel({ vendor: parsedVendor, sender: receipt.sender, subject: receipt.subject, fallback: 'Gmail receipt' });
  ...
  return buildReceiptRecord({ receipt, company, currency, flow, receivedAt, parsedVendor, parsedDate, dateValue });
};
