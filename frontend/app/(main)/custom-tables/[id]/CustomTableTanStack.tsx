...
function MobileView({ props: p, ctx }: ViewProps): React.JSX.Element {
  return (
    <MobileTableView
      isDark={ctx.isDark} rows={p.rows} orderedColumns={ctx.state.orderedColumns}
      selectedRowsSet={ctx.state.selectedRowsSet} selectedRowIds={p.selectedRowIds}
      allRowsSelectedMobile={ctx.state.allRowsSelectedMobile}
      someRowsSelectedMobile={ctx.state.someRowsSelectedMobile}
      loadingRows={p.loadingRows} isFullscreen={p.isFullscreen}
      onCreateRow={p.onCreateRow} onViewRow={p.onViewRow} onEditRow={p.onEditRow}
      onDeleteRow={p.onDeleteRow} onSelectAll={ctx.state.handleMobileSelectAll}
      onSelectRow={ctx.state.handleMobileSelectRow} onScroll={ctx.state.handleScroll}
      tableContainerRef={ctx.state.tableContainerRef} labels={ctx.mobileLabels}
...
function DesktopView({ props: p, ctx }: ViewProps): React.JSX.Element {
  return (
    <DesktopTableView
      isDark={ctx.isDark} isPrintMode={p.isPrintMode ?? false} isFullscreen={p.isFullscreen}
      table={ctx.state.table} virtualItems={ctx.state.virtualItems}
      rowVirtualizer={ctx.state.rowVirtualizer} stickyOffsets={ctx.state.stickyOffsets}
      colorPickerRowId={ctx.colorPickerRowId} colorPickerValue={ctx.state.colorPickerValue}
      colorPickerAnchorPosition={ctx.state.colorPickerAnchorPosition}
      loadingRows={p.loadingRows} onScroll={ctx.state.handleScroll}
      ...
      onCreateRow={p.onCreateRow} labels={ctx.commonLabels}
    />
  );
}
...
function propsToHookParams(props: CustomTableTanStackProps): UseCustomTableTanStackParams {
  return {
    rows: props.rows, columns: props.columns, selectedRowIds: props.selectedRowIds,
    columnWidths: props.columnWidths, stickyLeftColumnIds: props.stickyLeftColumnIds,
    stickyRightColumnIds: props.stickyRightColumnIds, loadingRows: props.loadingRows,
    hasMore: props.hasMore, onUpdateCell: props.onUpdateCell,
    onUpdateRowStyle: props.onUpdateRowStyle, onCreateRow: props.onCreateRow,
    onDeleteRow: props.onDeleteRow, onPersistColumnWidth: props.onPersistColumnWidth,
    onSelectedRowIdsChange: props.onSelectedRowIdsChange,
    onRenameColumnTitle: props.onRenameColumnTitle, onDeleteColumn: props.onDeleteColumn,
    onAddColumnClick: props.onAddColumnClick, onLoadMore: props.onLoadMore,
  };
}
...
export function CustomTableTanStack(props: CustomTableTanStackProps): React.JSX.Element {
  ...
  if (isMobile) return <MobileView props={props} ctx={ctx} />;
