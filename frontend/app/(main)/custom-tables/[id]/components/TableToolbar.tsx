...
const BTN_BASE_SX = { display: 'inline-flex', flexShrink: 0, alignItems: 'center', gap: { xs: 0.75, sm: 1 }, whiteSpace: 'nowrap', border: '1px solid #e5e7eb', px: { xs: 1.25, sm: 2 }, py: { xs: 0.5, sm: 0.75 }, fontSize: { xs: 11, sm: 12 }, fontWeight: 500, bgcolor: 'transparent', cursor: 'pointer', '&:disabled': { opacity: 0.5, cursor: 'not-allowed' } };
...
function TabsRow(p: P): React.JSX.Element {
  ...
  return (
    <Box sx={{ display: 'flex', width: '100%', alignItems: 'flex-end', justifyContent: 'space-between', gap: 1.5, borderBottom: '1px solid #f3f4f6', px: 1 }}>
      <Box component="button" type="button" onClick={p.handleBackNavigation} sx={{ display: 'inline-flex', flexShrink: 0, alignItems: 'center', gap: 0.75, pb: 1.5, fontSize: 14, fontWeight: 500, color: '#4b5563', bgcolor: 'transparent', border: 'none', cursor: 'pointer', '&:hover': { color: '#111827' } }}>
...
function SearchBox({ t, searchQuery, setSearchQuery }: SearchBoxProps): React.JSX.Element {
  return (
    <Box sx={{ position: 'relative' }}>
      ...
      <input placeholder={tx(t, ['actions', 'searchPlaceholder'], 'Search')} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{ paddingLeft: 36, paddingRight: 16, paddingTop: 8, paddingBottom: 8, fontSize: 14, width: 192, border: '1px solid #e5e7eb', background: 'var(--card-bg)', outline: 'none' }} />
    </Box>
  );
}
...
function BulkActionsRow(p: P): React.JSX.Element {
  ...
  const btnColor = can ? '#4b5563' : '#9ca3af';
  ...
  return (<Box sx={{ mt: 1.5, width: '100%', px: 1, pb: 1.5 }}>
    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1, overflowX: 'auto' }}>
      <Box sx={{ display: 'flex', minWidth: 0, flexWrap: 'nowrap', alignItems: 'center', gap: { xs: 0.75, sm: 1 } }}>
        <Box component="button" type="button" onClick={() => p.markSelectedRowsPaid(true)} disabled={!can} sx={{ ...BTN_BASE_SX, color: btnColor, '&:hover': { bgcolor: '#f9fafb' } }}>
          <CheckCircle style={{ width: 14, height: 14, color: paidColor }} /><span>{markPaidLabel}</span>
        </Box>
        <Box component="button" type="button" onClick={() => p.markSelectedRowsPaid(false)} disabled={!can} sx={{ ...BTN_BASE_SX, color: btnColor, '&:hover': { bgcolor: '#f9fafb' } }}>
          <XCircle style={{ width: 14, height: 14, color: unpaidColor }} /><span>{markUnpaidLabel}</span>
        </Box>
        <Box component="button" onClick={p.handlePrintTable} sx={{ ...BTN_BASE_SX, color: '#4b5563', '&:hover': { bgcolor: '#f9fafb', color: '#111827' } }}>
          <Printer className="h-3.5 w-3.5" /><span>{tx(p.t, ['actions', 'print'], 'Print')}</span>
        </Box>
        <Box component="button" onClick={() => p.openBulkDeleteModal(p.selectedRowIds)} disabled={p.selectedRowIds.length === 0} sx={{ ...BTN_BASE_SX, color: '#4b5563', '&:hover': { borderColor: '#fecaca', bgcolor: '#fef2f2', color: '#dc2626' } }}>
...
export function TableToolbar(p: P): React.JSX.Element {
  ...
  const containerSx = p.isFullscreen ? { position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, bgcolor: 'background.paper', px: { xs: 2, sm: 3 }, pt: 2.5, borderLeft: '1px solid #e5e7eb', borderRight: '1px solid #e5e7eb', borderTop: '1px solid #e5e7eb', ...(isColTab ? { bottom: 0, overflowY: 'auto', pb: 3 } : { pb: 0 }) } : { mb: 0, display: 'flex', flexDirection: 'column', gap: 0 };
  return (
