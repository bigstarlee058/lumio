...
export function computeOrderedVisibleColumns(
  ...
  return orderedKeys.map(key => columnsByKey.get(key)).filter(Boolean).filter(col => !hiddenSet.has((col as CustomTablePageColumn).key)) as CustomTablePageColumn[];
}
...
export function findSpecialColumnKeys(cols: CustomTablePageColumn[]): { dateColKey: string | null; counterpartyColKey: string | null } {
  ...
  const counterpartyColKey = cols.find(c => re.test(`${c.title ?? ''} ${c.key ?? ''}`))?.key ?? null;
  return { dateColKey, counterpartyColKey };
}

export function computeGridWidths(cols: CustomTablePageColumn[], getColumnWidth: (key: string) => number): Record<string, number> {
  const next: Record<string, number> = {};
  for (const col of cols) next[col.key] = getColumnWidth(col.key);
  return next;
}
