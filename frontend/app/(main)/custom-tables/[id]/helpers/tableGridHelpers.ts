...
export function mergeAndDedupeRows(prev: CustomTableGridRow[], next: CustomTableGridRow[], shouldReset: boolean): CustomTableGridRow[] {
...
export function shouldSkipLoad(tableId: string | null, shouldReset: boolean, loadingRef: { current: boolean }): boolean {
  return !tableId || (!shouldReset && loadingRef.current);
}

interface LoadRowsOpts { reset?: boolean; filtersParam?: string; }

function getLoadCursor(shouldReset: boolean, rowsRef: { current: CustomTableGridRow[] }): number | undefined {
  if (shouldReset) return undefined;
  return rowsRef.current[rowsRef.current.length - 1]?.rowNumber;
}

function getLoadFilters(opts: LoadRowsOpts | undefined, filtersRef: { current: string | undefined }): string | undefined {
...
function finalizeLoad({ requestId, seqRef, loadingRef, setLoadingRows, abortRef, controller }: FinalizeLoadArgs): void {
  if (requestId === seqRef.current) { loadingRef.current = false; setLoadingRows(false); }
...
export async function performLoadRows({ tableId, shouldReset, opts, controller, requestId, seqRef, rowsRef, filtersRef, loadingRef, abortRef, setLoadingRows, setRows, setHasMore, loadRowsFailedMessage }: PerformLoadArgs): Promise<void> {
  try {
    const cursor = getLoadCursor(shouldReset, rowsRef);
    const filters = getLoadFilters(opts, filtersRef);
    const response = await apiClient.get(`/custom-tables/${tableId}/rows`, { signal: controller.signal, params: { cursor, limit: 50, filters } });
    ...
    if (!isAbortError(error)) { console.error('Failed to load rows:', error); toast.error(loadRowsFailedMessage); }
  } finally {
