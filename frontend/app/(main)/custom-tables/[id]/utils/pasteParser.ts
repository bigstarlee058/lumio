...
export const matchFieldByName = (raw: string): PasteFieldKey | null => {
  ...
  for (const [field, aliases] of Object.entries(PASTE_FIELD_ALIASES)) {
    if (aliases.some(alias => normalized === normalizeToken(alias))) return field as PasteFieldKey;
...
export const parseDateCell = (raw: string): ParsedCell<string> => {
  ...
  if (!validateParsedDate(parsed, parts)) return { value: null, error: true };
...
const normalizeSingleSeparator = (stripped: string, sepIndex: number): string => {
  ...
  if (digitsAfter === 3) return stripped.replace(/[.,]/g, '');
  return stripped.replace(/[.,]/g, (match, offset: number) => (offset === sepIndex ? '.' : ''));
};
...
const normalizeMultipleSeparators = (stripped: string): string => {
  ...
  return stripped.replace(/[.,]/g, (match, offset: number) => (offset === decimalPos ? '.' : ''));
};

const normalizeStripped = (stripped: string): string => {
  const separators = stripped.match(/[.,]/g) || [];
  if (separators.length === 1) return normalizeSingleSeparator(stripped, stripped.search(/[.,]/));
  if (separators.length > 1) return normalizeMultipleSeparators(stripped);
...
export const parseNumberCell = (raw: string): ParsedCell<number> => {
  ...
  if (!/^[-+]?\d+(\.\d+)?$/.test(normalized)) return { value: null, error: true };
...
export const resolveCurrencyCode = (raw: string): string | null => {
  ...
  for (const [code, aliases] of Object.entries(CURRENCY_ALIASES)) {
    if (aliases.some(alias => normalizeCurrencyToken(alias) === normalized)) return code;
  }
  return null;
};
...
const matchCurrencyInOptions = (
  ...
  if (matchIndex !== -1) return options[matchIndex];
  if (resolved) {
    const resolvedIndex = normalizedOptions.indexOf(normalizeToken(resolved));
    if (resolvedIndex !== -1) return options[resolvedIndex];
  }
  return null;
};
...
export const parseCurrencyCell = (raw: string, options?: string[]): ParsedCell<string> => {
  ...
  if (options?.length) {
    ...
    if (match) return { value: match, error: false };
...
const PAID_NEGATIVE = [
  'false', '0', 'no', 'n', 'f', 'нет', 'неоплачено', 'не оплачено', 'не оплачен', 'unpaid',
];

export const parsePaidCell = (raw: string): ParsedCell<boolean> => {
  const trimmed = raw.trim().toLowerCase();
  if (!trimmed) return { value: null, error: false };
  if (PAID_POSITIVE.includes(trimmed)) return { value: true, error: false };
  if (PAID_NEGATIVE.includes(trimmed)) return { value: false, error: false };
  return { value: null, error: true };
};
