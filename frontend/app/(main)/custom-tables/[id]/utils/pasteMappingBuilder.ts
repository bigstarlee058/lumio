...
const indexToken = ({ token, col, field, columnNameMap, columnNameToField }: IndexArgs): void => {
  ...
  if (field) columnNameToField.set(token, field);
};
...
export const buildColumnMaps = (orderedColumns: PasteColumn[]): ColumnMaps => {
  ...
  for (const col of orderedColumns) {
    ...
    if (col.title) indexToken({ token: normalizeToken(col.title), col, field, columnNameMap, columnNameToField });
    if (col.key) indexToken({ token: normalizeToken(col.key), col, field, columnNameMap, columnNameToField });
    if (field && !fieldToColumn.has(field)) fieldToColumn.set(field, col);
  }
...
const mapByNameMatch = (
  ...
  if (!match || ctx.usedExisting.has(match.key)) return null;
...
const buildExistingColumnMapping = (
  ...
  if (!selection.columnKey) return null;
...
const resolveNewField = (selection: PasteMappingSelection, resolvedTitle: string): PasteFieldKey => {
  if (selection.field === null) return 'comment';
...
const buildNewColumnMapping = ({ sourceColumn, selection, defaults }: BuildNewArgs): PasteColumnMapping => {
