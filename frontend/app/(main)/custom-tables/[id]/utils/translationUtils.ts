...
function resolveSegment(current: unknown, segment: string): unknown {
  ...
  if (!(segment in (current as Obj))) return undefined;
  return (current as Obj)[segment];
}

function extractStringValue(current: unknown, fallback: string): string {
  if (typeof current === 'string') return current;
  if (typeof current !== 'object' || current === null || !('value' in (current as Obj))) return fallback;
  const value = (current as { value?: unknown }).value;
  return typeof value === 'string' ? value : fallback;
}

export function getTranslationValue({ root, path, fallback = '' }: GetTranslationValueParams): string {
