...
import type { DeleteColumnFn, DeleteRowFn, OpenColorPickerFn, RenameColumnFn, UpdateCellFn } from './columnDefinitions.types';
...
interface RenderDataCellParams {
  row: TRow; column: TColumn; table: TTable;
  col: CustomTableColumn; onUpdateCell: UpdateCellFn;
}

interface CellCommonProps {
  row: TRow; column: TColumn; table: TTable;
  cellType: CustomTableColumn['type']; onUpdateCell: UpdateCellFn;
  style: React.CSSProperties; options?: string[];
}
...
function renderDataCell({ row, column, table, col, onUpdateCell }: RenderDataCellParams): React.JSX.Element {
  const baseStyle = col.style?.cell ?? {};
  const cellStyle = getCellStyle(row.original, col.key, baseStyle);
  const commonProps: CellCommonProps = { row, column, table, cellType: col.type, onUpdateCell, style: cellStyle, options: col.config?.options };
  ...
  return {
    ...
    header: ({ table }) => (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Checkbox aria-label="Select all rows" checked={table.getIsAllRowsSelected()} indeterminate={table.getIsSomeRowsSelected()} onChange={table.getToggleAllRowsSelectedHandler()} />
      </div>
    ),
    size: 44, minSize: 44, maxSize: 44, enableResizing: false, enableSorting: false,
    cell: ({ row }) => (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Checkbox aria-label={`Select row ${row.original.rowNumber}`} checked={row.getIsSelected()} disabled={!row.getCanSelect()} onChange={row.getToggleSelectedHandler()} />
      </div>
    ),
  };
}
...
interface DataColumnParams {
  col: CustomTableColumn; columnWidths: Record<string, number>;
  onUpdateCell: UpdateCellFn; onRenameColumnTitle: RenameColumnFn; onDeleteColumn?: DeleteColumnFn;
}
function buildDataColumn({ col, columnWidths, onUpdateCell, onRenameColumnTitle, onDeleteColumn }: DataColumnParams): ColumnDef<CustomTableGridRow> {
  const icon = typeof col.config?.icon === 'string' ? col.config.icon : null;
  return {
    id: col.key,
    header: ({ column, table }) => (
      <EditableHeader column={column} table={table} title={col.title} icon={icon} onRename={onRenameColumnTitle} onDelete={onDeleteColumn} />
    ),
    size: columnWidths[col.key] || 180, minSize: 80, maxSize: 1200, enableResizing: true,
    cell: ({ row, column, table }) => renderDataCell({ row, column, table, col, onUpdateCell }),
  };
}

interface ActionsColumnParams {
  actionsHeaderLabel: string; colorTooltipLabel: string; deleteLabel: string;
  onOpenColorPicker: OpenColorPickerFn; onDeleteRow: DeleteRowFn;
}
...
function buildAddColumnButton(onAddColumnClick?: () => void): ColumnDef<CustomTableGridRow> {
  return {
    ...
    size: 50, minSize: 50, maxSize: 50, enableResizing: false,
    cell: () => null,
  };
}

export function buildColumns({ orderedColumns, columnWidths, onUpdateCell, onRenameColumnTitle, onDeleteColumn, onAddColumnClick, onOpenColorPicker, onDeleteRow, actionsHeaderLabel, colorTooltipLabel, deleteLabel }: BuildColumnsParams): ColumnDef<CustomTableGridRow>[] {
  return [
    buildSelectColumn(),
    buildRowNumberColumn(),
    ...orderedColumns.map(col => buildDataColumn({ col, columnWidths, onUpdateCell, onRenameColumnTitle, onDeleteColumn })),
    buildActionsColumn({ actionsHeaderLabel, colorTooltipLabel, deleteLabel, onOpenColorPicker, onDeleteRow }),
