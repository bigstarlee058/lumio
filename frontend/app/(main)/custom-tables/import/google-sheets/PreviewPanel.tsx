...
const cellInputStyle: CSSProperties = {
  width: '100%', border: '1px solid #e5e7eb', background: 'var(--card-bg)',
...
const SampleTableRow = ({ row, hasExtra }: SampleTableRowProps): JSX.Element => (
  <tr style={{ borderBottom: '1px solid #f3f4f6' }}>
    ...
    {hasExtra && <td style={{ padding: '4px 8px', color: '#6b7280' }}>…</td>}
  </tr>
);
      ...
      <table style={{ minWidth: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
            <th style={{ padding: '8px', textAlign: 'left', fontWeight: 600, color: '#374151' }}>{t.preview.rowHeader}</th>
            {preview.columns.slice(0, 12).map(c => (
              <th key={c.index} style={{ padding: '8px', textAlign: 'left', fontWeight: 600, color: '#374151' }}>{c.title}</th>
            ))}
            {hasExtra && <th style={{ padding: '8px', textAlign: 'left', fontWeight: 600, color: '#6b7280' }}>…</th>}
          </tr>
        </thead>
        <tbody>
...
export const PreviewPanel = ({ preview, t: tRaw }: PreviewPanelProps): JSX.Element => {
  ...
  return (
    <Box sx={{ border: '1px solid #e5e7eb', bgcolor: 'background.paper', p: 2 }} data-tour-id="gs-import-preview-panel">
      ...
      {!preview
        ? <Box sx={{ mt: 2, border: '1px dashed #e5e7eb', bgcolor: '#f9fafb', p: 3, fontSize: 14, color: '#4b5563' }}>{t.preview.hint}</Box>
...
const ColumnRow = ({ col, isLast, onUpdate, t }: ColumnRowProps): JSX.Element => (
  <tr style={{ borderBottom: isLast ? 'none' : '1px solid #f3f4f6' }}>
    ...
    <td style={{ padding: '8px 12px', color: '#6b7280' }}>{col.a1}</td>
    <td style={{ padding: '8px 12px' }}>
    ...
    <Box sx={{ mt: 2, overflowX: 'auto' }}>
      <table style={{ minWidth: '100%', fontSize: 14, borderCollapse: 'collapse' }}>
        <thead style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
          <tr>
            <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: '#374151', width: 84 }}>{t.columns.tableHeaders.enabled}</th>
            <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: '#374151', width: 80 }}>A1</th>
            <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: '#374151' }}>{t.columns.tableHeaders.name}</th>
            <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: '#374151', width: 180 }}>{t.columns.tableHeaders.type}</th>
          </tr>
        </thead>
        <tbody>
  ...
  return (
    <Box sx={{ border: '1px solid #e5e7eb', bgcolor: 'background.paper', p: 2 }} data-tour-id="gs-import-columns-panel">
      <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 1.5 }}>
        ...
        {preview && (
          <Box component="button" onClick={() => setColumns(prev => prev.map(c => ({ ...c, include: true })))}
      ...
      {!preview
        ? <Box sx={{ mt: 2, border: '1px dashed #e5e7eb', bgcolor: '#f9fafb', p: 3, fontSize: 14, color: '#4b5563' }}>{t.columns.appearAfterPreview}</Box>
