...
const JobProgress = ({
  ...
  <Box
    sx={{ mt: 1.5, border: '1px solid var(--border-color)', bgcolor: 'background.paper', p: 1.5 }}
  >
    ...
    >
      <Box
        sx={{
          ...
          border: '1px solid var(--border-color)',
          bgcolor: selected?.color || 'var(--muted)',
        }}
      >
        {selected?.icon ? (
...
const CommitButton = ({
  ...
  onCommit,
  t,
}: CommitButtonProps): React.JSX.Element => (
  <Box
    component="button"
    onClick={onCommit}
...
export const ResultCard = ({
  ...
  return (
    ...
    >
      <Typography
        style={{ fontSize: 14, fontWeight: 600, color: 'var(--foreground)', marginBottom: 12 }}
      >
        {t.result.title}
      </Typography>
      <TableForm
      ...
      >
        <Checkbox
          checked={importData}
          onCheckedChange={v => setImportData(v as boolean)}
          className="h-5 w-5"
        />
      ...
      {!preview && (
        <Typography style={{ marginTop: 8, fontSize: 12, color: 'var(--muted-foreground)' }}>
