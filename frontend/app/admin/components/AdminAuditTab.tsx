...
const ENTITY_TYPES = [
  'transaction', 'statement', 'receipt', 'category', 'rule', 'workspace',
  'integration', 'table_row', 'table_cell', 'branch', 'wallet', 'custom_table',
  'custom_table_column',
] as const;

const ACTIONS = [
  'create', 'update', 'delete', 'import', 'link', 'unlink',
  'match', 'unmatch', 'apply_rule', 'rollback', 'export',
] as const;
...
function AuditSelectFilters({ filters, labels, onFilterChange }: FilterProps): React.JSX.Element {
  return (
    <>
      <TextField select label={labels.filterEntityType} size="small" value={filters.entityType || ''}
      ...
      <TextField select label={labels.filterAction} size="small" value={filters.action || ''}
        onChange={(e) => onFilterChange({ key: 'action', value: e.target.value })}
        SelectProps={{ native: true }}>
        <option value="">{labels.filterAll}</option>
        {ACTIONS.map((action) => <option key={action} value={action}>{action}</option>)}
      </TextField>
      <TextField select label={labels.filterSeverity} size="small" value={filters.severity || ''}
...
function AuditFilters({ filters, labels, onFilterChange }: FilterProps): React.JSX.Element {
  return (
    <Paper variant="outlined" className="lumio-audit-panel__filters" data-tour-id="admin-audit-filters">
      <Typography variant="subtitle2" gutterBottom>{labels.filtersTitle}</Typography>
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2 }}>
        <AuditSelectFilters filters={filters} labels={labels} onFilterChange={onFilterChange} />
        <TextField label={labels.filterUser} size="small" value={filters.actorLabel || ''}
...
function AuditResults({ auditLogs, auditLoading, auditError, auditPage, auditLimit, auditTotal, labels, onSelectEvent, onPageChange }: {
  auditLogs: AuditEvent[]; auditLoading: boolean; auditError: string | null;
  auditPage: number; auditLimit: number; auditTotal: number;
  labels: AdminAuditTabProps['labels'];
  onSelectEvent: AdminAuditTabProps['onSelectEvent'];
  onPageChange: AdminAuditTabProps['onPageChange'];
}): React.JSX.Element {
  if (auditError) {
    return <Box sx={{ mb: 2, p: 2, bgcolor: 'error.light', color: 'error.contrastText' }}>{auditError}</Box>;
  }
  if (auditLoading) {
    return <Paper variant="outlined" sx={{ p: 3 }}><Typography variant="body2" color="text.secondary">{labels.loading}</Typography></Paper>;
  }
  if (auditLogs.length === 0) {
    return <Paper variant="outlined" sx={{ p: 3 }}><Typography variant="body2" color="text.secondary">{labels.empty}</Typography></Paper>;
  }
  return (
    <AuditEventTable events={auditLogs} onSelect={onSelectEvent}
      page={auditPage} limit={auditLimit} total={auditTotal} onPageChange={onPageChange} />
  );
}

export function AdminAuditTab({ auditLogs, auditTotal, auditPage, auditLimit, auditFilters,
  auditLoading, auditError, selectedAuditEvent, auditDrawerOpen, labels,
  onFilterChange, onPageChange, onSelectEvent, onCloseDrawer }: AdminAuditTabProps): React.JSX.Element {
  return (
    <Box className="lumio-audit-panel">
      <Box className="lumio-audit-panel__header">
        <Typography variant="h6" component="h2">{labels.title}</Typography>
        <Typography variant="body2" color="text.secondary">{labels.helper}</Typography>
      </Box>
      <Box className="lumio-audit-panel__layout">
        ...
        <Box className="lumio-audit-panel__results">
          <AuditResults auditLogs={auditLogs} auditLoading={auditLoading} auditError={auditError}
            auditPage={auditPage} auditLimit={auditLimit} auditTotal={auditTotal}
            labels={labels} onSelectEvent={onSelectEvent} onPageChange={onPageChange} />
        </Box>
      </Box>
