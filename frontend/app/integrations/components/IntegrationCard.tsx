...
const externalLinkStyle: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', padding: '6px 12px', fontSize: 12, fontWeight: 500, border: '1px solid var(--border-color)', borderRadius: tokens.radius.md, color: 'var(--foreground)', textDecoration: 'none' };
...
function ExternalActionLink({ action }: { action: IntegrationAction }): React.JSX.Element {
  return (
    <a href={action.href} target="_blank" rel="noreferrer" onClick={(e): void => e.stopPropagation()} style={externalLinkStyle}>
      {action.label}
      <ExternalLink style={{ height: 12, width: 12, marginLeft: 4, color: 'var(--muted-foreground)' }} />
    </a>
  );
}
...
function DisabledPrimaryButton({ action }: { action: IntegrationAction }): React.JSX.Element {
  return (
    <button type="button" disabled style={{ display: 'inline-flex', alignItems: 'center', padding: '6px 12px', fontSize: 12, fontWeight: 500, border: '1px solid var(--border-color)', borderRadius: tokens.radius.md, color: 'var(--muted-foreground)', cursor: 'not-allowed', background: 'transparent' }} onClick={(e): void => e.stopPropagation()}>
      {action.label}
    </button>
  );
}
...
function SecondaryLink({ action }: { action: IntegrationAction }): React.JSX.Element {
  return (
    <Link href={action.href} onClick={(e): void => e.stopPropagation()} style={{ display: 'inline-flex', alignItems: 'center', padding: '6px 12px', fontSize: 12, fontWeight: 500, border: '1px solid var(--border-color)', borderRadius: tokens.radius.md, color: 'var(--foreground)', textDecoration: 'none' }}>
      {action.label}
    </Link>
  );
}
...
function IntegrationBadge({ recommended, badge }: { recommended: boolean; badge: React.ReactNode }): React.JSX.Element {
  if (recommended) {
    return (
      <Box component="span" sx={{ display: 'inline-flex', alignItems: 'center', fontSize: 12, fontWeight: 600, px: 1, py: 0.25, bgcolor: 'var(--color-warning-soft-bg)', color: 'var(--color-warning-soft-text)', border: '1px solid var(--color-warning-soft-border)', borderRadius: tokens.radius.sm }}>
        <Star style={{ height: 12, width: 12, marginRight: 4 }} />{badge}
      </Box>
    );
  }
  return (
    <Box component="span" sx={{ display: 'inline-flex', alignItems: 'center', fontSize: 12, fontWeight: 600, px: 1, py: 0.25, bgcolor: 'var(--color-success-soft-bg)', color: 'var(--color-success-soft-text)', border: '1px solid var(--color-success-soft-border)', borderRadius: tokens.radius.sm }}>
  ...
  return (
    <Box sx={{ flex: 1 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        <Typography style={{ fontSize: 18, fontWeight: 600, color: 'var(--foreground)' }}>{item.name}</Typography>
        <IntegrationBadge recommended={item.recommended} badge={item.badge} />
      </Box>
      <Typography style={{ fontSize: 14, color: 'var(--text-secondary)', marginTop: 4 }}>{item.description}</Typography>
...
function makeCardSx(clickable: boolean): object {
  return { border: '1px solid var(--border-color)', borderRadius: tokens.radius.lg, p: 2, bgcolor: 'background.paper', boxShadow: '0 1px 2px 0 rgba(0,0,0,0.05)', cursor: clickable ? 'pointer' : 'default', '&:hover': { boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }, '&:focus-visible': { outline: '2px solid var(--color-primary)', outlineOffset: 2 } };
}
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1.5 }}>
        <Box sx={{ p: 1, borderRadius: tokens.radius.sm, bgcolor: 'var(--muted)', border: '1px solid var(--muted)', display: 'flex' }}>{item.icon}</Box>
...
export function CategoryDivider({ label }: { label: React.ReactNode }): React.JSX.Element {
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
      ...
      <Box component="span" sx={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--muted-foreground)', px: 1.5, py: 0.5, border: '1px solid var(--border-color)', borderRadius: tokens.radius.sm, bgcolor: 'background.paper' }}>{label}</Box>
      <Box sx={{ height: '1px', flex: 1, bgcolor: 'var(--border-color)' }} />
    </Box>
  );
}
